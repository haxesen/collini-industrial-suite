...
const translations = {
  hu: {
    ...
    hubTitle: 'Collini Industrial Suite',
...
function App() {
  ...
  useEffect(() => {
    if (view === 'logbook' || view === 'info_wall') {
        ...
        <div className="ticker-content">
          {activeInfos.map((info, idx) => (
            <div key={info.id} className={`ticker-item priority-${info.priority}`}>
              <span className="ticker-dept">[{info.department}]</span>
              <span className="ticker-msg">{info.message}</span>
                ...
                <table className="logbook-table">
                  ...
                  <tbody>
                    {filteredAndSortedEntries.map(entry => (
                      <tr key={entry.id} className={`row-prio-${entry.prio.split('_')[1] || entry.prio}`}>
                        ...
                        <td className="col-status">
                          <div className={`status-badge-container ${entry.status.split('_')[1].toLowerCase()}`}>
                  ...
                  activeInfos.map(info => (
                    <div key={info.id} className={`info-card priority-${info.priority}`}>
                      <div className="info-card-header">
                        <span className="info-dept">{info.department}</span>
                      ...
                      <div className="info-card-footer">
                        <span className="info-author">{info.author}</span>
                      </div>
                    </div>
                  ))
                )}
